...
class ParticleFilterNode(LifecycleNode):
    ...
    def on_configure(self, state: LifecycleState) -> TransitionCallbackReturn:
        ...
        try:
            ...
            self._localized_publisher = self.create_publisher(
                PoseStamped, "localized_pose", qos_profile=10
            )
    ...
    def _publish_pose_estimate(self, x_h: float, y_h: float, theta_h: float) -> None:
        ...
        # Check if robot has been localized
        if self._localized:
            msg.pose.position.x = x_h
            msg.pose.position.y = y_h
            msg.pose.position.z = 0.0

            quaterion = euler2quat(0.0, 0.0, theta_h)
            msg.pose.orientation.x = quaterion[1]
            msg.pose.orientation.y = quaterion[2]
            msg.pose.orientation.z = quaterion[3]
            msg.pose.orientation.w = quaterion[0]
